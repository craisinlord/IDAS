...
public class GenericEndJigsawStructure<C extends IDASGenericConfig> extends GenericJigsawStructure<C> {
    ...
    protected static <CC extends IDASGenericConfig> boolean isLandFeatureChunk(PieceGeneratorSupplier.Context<CC> context) {
        return isGenericFeatureChunk(context) && getTerrainHeight(context.chunkPos(), context.chunkGenerator(), context.heightAccessor()) >= Math.min(GeneralUtils.getMaxTerrainLimit(context.chunkGenerator()), 50);
    }
    ...
    public static <CC extends IDASGenericConfig> Optional<PieceGenerator<CC>> generateLandPieces(PieceGeneratorSupplier.Context<CC> context) {
