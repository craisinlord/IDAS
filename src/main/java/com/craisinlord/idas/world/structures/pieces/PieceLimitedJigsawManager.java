...
import com.craisinlord.idas.utils.BoxOctree;
...
public class PieceLimitedJigsawManager {
    ...
    public static <C extends FeatureConfiguration> Optional<PieceGenerator<C>> assembleJigsawStructure(
        ...
        if(startPool == null || startPool.size() == 0) {
            IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent start pool in structure: {}  Crash is imminent", structureID);
            throw new RuntimeException("Integrated Dungeons and Structures: Empty or nonexistent start pool in structure: " + structureID + " Crash is imminent");
        }
        ...
        return Optional.of((structurePiecesBuilder, contextx) -> {
            ...
            Map<ResourceLocation, StructurePieceCountsManager.RequiredPieceNeeds> requiredPieces = IDAS.structurePieceCountsManager.getRequirePieces(structureID);
                ...
                if (attempts == 100) {
                    IDAS.LOGGER.error(
                            """
                                    ...
                                    -------------------------------------------------------------------
                                    Integrated Dungeons and Structures: Failed to create valid structure with all required pieces starting from this pool file: {}. Required pieces failed to generate the required amount are: {}
                                      ...
                                      please report the issue to Integrated Dungeons and Structures's dev with latest.log file!
        ...
        for(Object piece : components) {
            if(piece instanceof PoolElementStructurePiece) {
                ...
                if(poolElement instanceof SinglePoolElement) {
                    ResourceLocation pieceID = ((SinglePoolElementAccessor) poolElement).idas_getTemplate().left().orElse(null);
    ...
    public static final class Assembler {
        ...
        public <C extends FeatureConfiguration> Assembler(ResourceLocation structureID, Registry<StructureTemplatePool> poolRegistry, int maxDepth, PieceGeneratorSupplier.Context<C> context, List<? super PoolElementStructurePiece> structurePieces, Random rand, Map<ResourceLocation, StructurePieceCountsManager.RequiredPieceNeeds> requiredPieces, int maxY, int minY, Set<ResourceLocation> poolsThatIgnoreBounds) {
            ...
            this.maximumPieceCounts = new HashMap<>(IDAS.structurePieceCountsManager.getMaximumCountForPieces(structureID));
        ...
        public void generatePiece(PoolElementStructurePiece piece, MutableObject<BoxOctree> boxOctree, int minY, int depth, boolean doBoundaryAdjustments, LevelHeightAccessor heightLimitView) {
            ...
            for (StructureTemplate.StructureBlockInfo jigsawBlock : pieceJigsawBlocks) {
                ...
                if (!(poolOptional.isPresent() && (poolOptional.get().size() != 0 || Objects.equals(jigsawBlockPool, Pools.EMPTY.location())))) {
                    IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockPool, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).idas_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
                ...
                if (!(fallbackOptional.isPresent() && (fallbackOptional.get().size() != 0 || Objects.equals(jigsawBlockFallback, Pools.EMPTY.location())))) {
                    IDAS.LOGGER.warn("Integrated Dungeons and Structures: Empty or nonexistent pool: {} which is being called from {}", jigsawBlockFallback, pieceBlueprint instanceof SinglePoolElement ? ((SinglePoolElementAccessor) pieceBlueprint).idas_getTemplate().left().get() : "not a SinglePoolElement class");
                    continue;
                }
                ...
                if (depth != this.maxDepth) {
                    StructurePoolElement generatedPiece = this.processList(new ArrayList<>(((StructurePoolAccessor)poolOptional.get()).idas_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop, heightLimitView, false);
                ...
                this.processList(new ArrayList<>(((StructurePoolAccessor)fallbackOptional.get()).idas_getRawTemplates()), doBoundaryAdjustments, jigsawBlock, jigsawBlockTargetPos, pieceMinY, jigsawBlockPos, octreeToUse, piece, depth, targetPieceBoundsTop, heightLimitView, ignoreBounds);
            }
        }
            ...
            while (candidatePieces.size() > 0) {
                ...
                if (pieceNeededToSpawn.isPresent()) {
                    for (int i = 0; i < candidatePieces.size(); i++) {
                        ...
                        if (candidatePiece instanceof SinglePoolElement && ((SinglePoolElementAccessor) candidatePiece).idas_getTemplate().left().get().equals(pieceNeededToSpawn.get())) { // Condition 1
                ...
                if(candidatePiece instanceof SinglePoolElement) {
                    pieceName = ((SinglePoolElementAccessor) candidatePiece).idas_getTemplate().left().get();
