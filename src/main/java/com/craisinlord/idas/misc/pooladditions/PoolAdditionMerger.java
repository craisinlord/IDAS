...
import com.craisinlord.idas.mixin.structures.StructureManagerAccessor;
...
public final class PoolAdditionMerger {
    ...
    public static void mergeAdditionPools(final ServerAboutToStartEvent event) {
        ResourceManager resourceManager = ((StructureManagerAccessor) event.getServer().getStructureManager()).idas_getResourceManager();
            ...
            for (JsonElement jsonElement : entry.getValue()) {
                try {
                    ...
                    IDAS.LOGGER.error("""
                            ...
                            Integrated Dungeons and Structures: Pool Addition json failed to be parsed.
    ...
    private static void mergeIntoExistingPool(AdditionalStructureTemplatePool feedingPool, StructureTemplatePool gluttonyPool, StructureManager structureManager) {
        ...
        List<StructurePoolElement> elements = new ArrayList<>(((StructurePoolAccessor) gluttonyPool).idas_getTemplates());
        ...
        for(StructurePoolElement element : elements) {
            if(element instanceof SinglePoolElement singlePoolElement) {
                ...
                if(structureTemplate.isEmpty()) {
                    IDAS.LOGGER.error("(Integrated Dungeons and Structures POOL MERGER) Found an entry in {} that points to the non-existent nbt file called {}", feedingPool.getName(), nbtID.get());
                }
            }
    ...
    private static void logBadData(ResourceLocation poolPath, String messageString) {
        IDAS.LOGGER.error("(Integrated Dungeons and Structures POOL MERGER) Failed to parse {} additions file. Error is: {}", poolPath, messageString);
    }
            ...
            super(resourceLocation, resourceLocation2, rawTemplatesWithConditions.stream().filter(triple -> {
                if(triple.condition().isPresent()) {
                    ...
                    if(optionalSupplier.isPresent()) {
                        ...
                        IDAS.LOGGER.error("Error: Found " + resourceLocation + " entry has a condition that does not exist.");
                    }
                }
                return true;
